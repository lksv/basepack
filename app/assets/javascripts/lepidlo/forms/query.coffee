class Lepidlo.QueryForm
  ...
  select_predicate: (name, value, options, klass) ->
    JST["lepidlo/forms/query/predicate"]
      name: name
      value: value
      options: options
      klass: klass
  ...
  append: (field_label, field_name, field_type, field_value, field_predicate, field_template, index) ->
    ...
    if field_template
      ...
      switch field_type
        ...
        when "date", "datetime", "timestamp"
          ...
          additional_control = JST["lepidlo/forms/query/date"]
            name: value_name,
            value: field_value,
    ...
    content = JST["lepidlo/forms/query/filter"]
      index: index,
    ...
    form = new Lepidlo.Form(@$filters.find(".filter-#{index}"))
    form.bind()
